...
#define HALERR(fmt, ...)					\
    rtapi_print_msg(RTAPI_MSG_ERR,__FUNCTION__,__LINE__,	\
		    "HAL error:", fmt, ## __VA_ARGS__)
#define HALWARN(fmt, ...)					\
    rtapi_print_msg(RTAPI_MSG_WARN,__FUNCTION__,__LINE__,	\
		    "HAL WARNING:", fmt, ## __VA_ARGS__)
#define HALINFO(fmt, ...)					\
    rtapi_print_msg(RTAPI_MSG_INFO,__FUNCTION__,__LINE__,	\
		    "HAL info:", fmt, ## __VA_ARGS__)

#define HALDBG(fmt, ...)					\
    rtapi_print_msg(RTAPI_MSG_DBG,__FUNCTION__,__LINE__,	\
		    "HAL:", fmt, ## __VA_ARGS__)
...
#define CHECK_STR(name)							\
    do {								\
	if ((name) == NULL) {						\
	    rtapi_print_msg(RTAPI_MSG_ERR,__FUNCTION__, __LINE__,"HAL error:", \
